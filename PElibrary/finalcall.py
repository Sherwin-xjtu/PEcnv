...
def do_finalcall(cnarr, variants=None, method="threshold", ploidy=2, purity=None,
    ...
    if purity and purity < 1.0:
        ...
        logging.info("finalcalling copy number with clonal ploidy %d", ploidy)
    ...
    if method == "threshold":
        ...
        logging.info("finalcalling copy number with thresholds: %s",
                     ", ".join(tokens))
...
def absolute_threshold(cnarr, ploidy, thresholds, is_reference_male):
    """finalcall integer copy number using hard thresholds for each level.

    Integer values are assigned for log2 ratio values less than each given
    threshold value in sequence, counting up from zero.
    Above the last threshold value, integer copy numbers are finalcalled assuming
    full purity, diploidy, and rounding up.

    Default thresholds follow this heuristic for finalcalling CNAs in a tumor sample:
...
def absolute_clonal(cnarr, ploidy, purity, is_reference_male, is_sample_female):
    ...
    for i, row in enumerate(cnarr):
        # TODO by_chromosome to reduce number of finalcalls to this
...
def _reference_expect_copies(chrom, ploidy, is_sample_female, is_reference_male):
    ...
    pecnv reference, while "expect" is the chromosome's neutral ploidy in the
