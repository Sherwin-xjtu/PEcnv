"""Supporting functions for the 'correct' command."""
import logging

import numpy as np
import pandas as pd

from . import descriptives, hyperparameters, adjusting


def do_correct(target_raw, offTarget_raw, reference,
           do_gc=True, do_edge=True, do_rmask=True, do_cluster=False):
    """Combine target and offTarget coverages and correct for biases."""
    # Load, recenter and GC-correct target & offTarget probes separately
    ...
    if len(anti_cnarr):
        # Combine target and offTarget bins
...
def load_adjust_coverages(cnarr, ref_cnarr, skip_low,
    ...
    # No corrections needed if there are no data rows (e.g. no offTargets)
    if not len(cnarr):
...
def match_ref_to_sample(ref_cnarr, samp_cnarr):
    """Filter the reference bins to match the sample (target or offTarget)."""
...
def center_by_window(cnarr, fraction, sort_key):
    ...
    #biases = adjusting.savgol(df['log2'], fraction)
    #biases = adjusting.wiener_filter(df['log2'], fraction)
    df['log2'] -= biases
...
def apply_weights(cnarr, ref_matched, log2_key, spread_key, epsilon=1e-4):
    ...
    log2_key : string
        ...
        may have a sufcorrect indicating the cluster, e.g. "log2_1".
    spread_key : string
