...
def get_regions(fasta_fname):
    """Find preprocessible sequence regions (those not masked out with 'N')."""
    with open(fasta_fname) as infile:
        ...
        for line in infile:
            if line.startswith('>'):
                ...
                run_start = None
                cursor = 0
                logging.info("%s: Scanning for preprocessible regions", chrom)
            else:
                line = line.rstrip()
                if 'N' in line:
                    ...
                    if run_start is None:
                        ...
                        run_start = cursor
                cursor += len(line)
        # Emit the last run if it's preprocessible (i.e. not a telomere)
        if run_start is not None:
            yield log_this(chrom, run_start, cursor)
...
def log_this(chrom, run_start, run_end):
    ...
    logging.info("\tpreprocessible region %s:%d-%d (size %d)",
