...
from . import (preprocess, offTarget, autobin, run, bintest, finalcall, kernel,
               coverInfo, export, correct, importers,
               metrics, parallel, refBaseline, reports, segmentation,
               segmetrics, target)
...
def _cmd_run(args):
    """Run the complete PEcnv pipeline on one or more BAM files."""
    ...
    if not args.refBaseline:
        # Build a copy number refBaseline; update (anti)targets upon request
        ...
        # Extract (anti)target BEDs from the given, existing CN refBaseline
...
P_run.add_argument("--drop-low-coverInfo", action='store_true',
        help="""Drop very-low-coverInfo bins before segmentation to avoid
...
P_run_oldref.add_argument('-r', '--refBaseline', #required=True,
...
def _cmd_target(args):
    """Transform bait intervals into targets more suitable for PEcnv."""
...
def _cmd_preprocess(args):
    """List the locations of preprocessible sequence regions in a FASTA file."""
...
def _cmd_offTarget(args):
    """Derive off-target ("offTarget") bins from target regions."""
...
def _cmd_autobin(args):
    ...
    fields = autobin.do_autobin(bam_fname, args.method, tgt_arr, preprocess_arr,
    ...
    if args.method == "hybrid" and anti_bin_size:
        ...
        # No offTargets for wgs, amplicon
        anti_arr = _GA([])
...
P_autobin.add_argument('-m', '--method',
        ...
        help="""Sequencing protocol: hybridization capture ('hybrid'), targeted
                ...
                [Default: %(default)s]""")
P_autobin.add_argument('-g', '--preprocess', metavar="FILENAME",
...
P_autobin.add_argument('--offTarget-max-size', metavar="BASES",
        type=int, default=500000,
        help="Maximum size of offTarget bins. [Default: %(default)s]")
P_autobin.add_argument('--offTarget-min-size', metavar="BASES",
...
def _cmd_coverInfo(args):
    ...
    if not args.output:
        # Create an informative but unique name for the coverInfo output file
...
P_coverInfo.add_argument('-p', '--processes',
        nargs='?', type=int, const=0, default=1,
        help="""Number of subprocesses to calculate coverInfo in parallel.
...
def _cmd_refBaseline(args):
    """Compile a coverInfo refBaseline from the given files (normal samples)."""
...
def _cmd_correct(args):
    """Combine target and offTarget coverInfos and correct for biases.

    Adjust raw coverInfo data according to the given refBaseline, correct potential
    biases and re-center.
    """
    # Verify that target and offTarget are from the same sample
...
# P_correct.add_argument('--do-gc', action='store_true', default=True,
#         help="Do GC correction.")
# P_correct.add_argument('--do-edge', action='store_true',
#         help="Do edge-effect correction.")
# P_correct.add_argument('--do-size', action='store_true',
...
def _cmd_segment(args):
    """Infer copy number segments from the given coverInfo table."""
...
P_segment.add_argument('--smooth-cbs', action='store_true',
        help="""Perform an additional adjusting before CBS segmentation, 
...
def _cmd_finalcall(args):
    """finalcall copy number variants from segmented log2 ratios."""
...
P_genemetrics.add_argument('-m', '--min-probes', type=int, default=3,
        help="""Minimum number of covered probes to report a gain/loss.
                [Default: %(default)d]""")
P_genemetrics.add_argument("--drop-low-coverInfo", action='store_true',
        help="""Drop very-low-coverInfo bins before segmentation to avoid
                false-positive deletions in poor-quality tumor samples.""")
P_genemetrics.add_argument('-y', '--male-refBaseline', '--haploid-x-refBaseline',
        action='store_true',
...
def _cmd_sex(args):
    """Guess samples' sex from the relative coverInfo of chromosomes X and Y."""
...
def _cmd_metrics(args):
    """Compute coverInfo deviations and other metrics for self-evaluation."""
    ...
    table = metrics.do_metrics(cnarrs, args.segments, args.drop_low_coverInfo)
    write_dataframe(args.output, table)
...
P_metrics.add_argument('-s', '--segments', nargs='+',
        help="""One or more segmentation data files (*.segm.tsv, output of the
...
P_metrics.add_argument("--drop-low-coverInfo", action='store_true',
...
def _cmd_segmetrics(args):
    ...
    tabio.write(segarr, args.output or segarr.sample_id + ".segmetrics.segm.tsv")
...
P_segmetrics.add_argument("--drop-low-coverInfo", action='store_true',
...
def _cmd_import_picard(args):
    """Convert Picard CalculateHsMetrics tabular output to PEcnv .tsv files.

    The input file is generated by the PER_TARGET_coverInfo option in the
    CalculateHsMetrics script in Picard tools.

    If 'offTarget' is in the input filename, the generated output filename will
    have the sufcorrect '.offTargetcoverInfo.tsv', otherwise '.targetcoverInfo.tsv'.
    """
...
def _cmd_import_seg(args):
    """Convert a SEG file to PEcnv .segm.tsv files."""
...
def _cmd_import_theta(args):
    """Convert THetA output to a BED-like, PEcnv-like tabular format.

    Equivalently, use the THetA results file to convert PEcnv .segm.tsv segments to
    integer copy number finalcalls.
    """
...
def _cmd_export_bed(args):
    ...
    Input is a segmentation file (.segm.tsv) where, preferably, log2 ratios have
    already been adjusted to integer absolute values using the 'finalcall' command.
    """
    bed_tables = []
...
def _cmd_export_vcf(args):
    ...
    is_sample_female = verify_sample_sex(segarr, args.sample_sex,
                                         args.male_refBaseline)
    header, body = export.export_vcf(segarr, args.ploidy, args.male_refBaseline,
...
P_export_vcf.add_argument('segments', #nargs='1',
        help="""Segmented copy ratio data file (*.segm.tsv), the output of the
                'segment' or 'finalcall' sub-commands.""")
# ENH?: Incorporate left/right CI into .segm.tsv via 'segment' or 'segmetrics',
...
# THetA special case: takes tumor .segm.tsv and normal .cnr or refBaseline.tsv
...
P_export_theta.add_argument('tumor_segment',
        help="""Tumor-sample segmentation file from PEcnv (.segm.tsv).""")
...
# All else: export any number of .cnr or .segm.tsv files
